...
# Install NVIDIA CUDA toolkit and drivers if not already installed
if ! command -v nvcc &> /dev/null; then
    echo "Installing NVIDIA CUDA toolkit and NVIDIA drivers"
    
    # Install NVIDIA CUDA toolkit
    sudo apt install -y nvidia-cuda-toolkit || handle_error "Failed to install nvidia-cuda-toolkit."
    
    # Install NVIDIA drivers (using a general approach for the latest recommended version)
    sudo apt install -y nvidia-driver-535 || handle_error "Failed to install nvidia-driver-535."
else
...
pip install torch torchvision torchaudio --index-url https://download.pytorch.org/whl/cu118 || handle_error "Failed to install PyTorch with CUDA 11.8."
...
if [ -f "$REQUIREMENTS_FILE" ]; then
    ...
    echo "requirements.txt not found at $REPO_DIR. Skipping dependency installation."
fi
